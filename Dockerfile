...
WORKDIR /gemstone
RUN wget --progress=dot https://downloads.gemtalksystems.com/pub/GemStone64/3.5.2/GemStone64Bit3.5.2-x86_64.Linux.zip
RUN unzip GemStone64Bit3.5.2-x86_64.Linux.zip \
        && chown -R gsadmin:users /gemstone \
        && rm GemStone64Bit3.5.2-x86_64.Linux.zip

# Create the /gemstone-keys directory and copy the community starter
# key there.
RUN mkdir /gemstone-keys \
        && cp /gemstone/GemStone64Bit3.5.2-x86_64.Linux/sys/community.starter.key  /gemstone-keys/gemstone.key \
        && chown -R gsadmin:users /gemstone-keys

# Copy our configuration file
COPY --chown=gsadmin:users system.conf /gemstone/GemStone64Bit3.5.2-x86_64.Linux/data
...
ENV GEMSTONE=/gemstone/GemStone64Bit3.5.2-x86_64.Linux

WORKDIR /gemstone/GemStone64Bit3.5.2-x86_64.Linux/install
RUN ./installgs

RUN cp -p /gemstone/GemStone64Bit3.5.2-x86_64.Linux/data/extent0.dbf /gemstone-data
